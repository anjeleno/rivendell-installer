...
if [ "$(whoami)" != "rd" ]; then
    ...
    if ! step_completed "set_hostname"; then set_hostname; fi
    if ! step_completed "create_rd_user"; then create_rd_user; fi
    if ! step_completed "copy_working_directory"; then copy_working_directory; fi
    if ! step_completed "configure_shell_profile"; then configure_shell_profile; fi
    prompt_reboot
    exit 1
fi
...
extract_mysql_password() {
    ...
    echo "MySQL password extracted: $MYSQL_PASSWORD"
...
enable_firewall() {
    ...
    sudo ufw allow 8000/t# filepath: /Users/brandon/git/rivendell-Cloud/rivendell-auto-install.sh
#!/bin/bash
# Rivendell Auto-Install Script
# Version: 0.20.1
# Date: 2025-03-15
